...
export default function SophiaChat({ sophia }: SophiaChatProps) {
    ...
    const loadHistory = async () => {
      ...
      if (data) {
        setMessages(
          data.map((log) => ({
            id: log.id,
            role: log.metadata_json?.role === 'user' ? 'user' : 'sophia',
  ...
  }, [sophia, scrollToBottom]);

  const sendMessage = async () => {
    if (!input.trim() || !sophia || sending) return;
    ...
    // Create a task for Sophia to process
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Rainha • {sophia.llm_provider}/{sophia.modelo} •{' '}
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-slate-600 mt-1">
                A Rainha conhece tudo sobre a Doctor Auto. Ela observa, aprende e pode criar princesas especializadas.
              </p>
            </div>
            <div className="flex flex-wrap gap-2 mt-3 max-w-md justify-center">
              {[
                'Como estao nossas metricas hoje?',
                'Crie uma princesa de follow-up',
      ...
      <div className="border-t border-slate-700/60 px-5 py-3">
        <div className="flex items-center gap-3">
          <input
            ...
            placeholder={
              sophia.status === 'online'
                ? 'Digite uma mensagem para Sophia...'
                : 'Sophia esta offline. Ligue-a para conversar.'
            }
            disabled={sending}
